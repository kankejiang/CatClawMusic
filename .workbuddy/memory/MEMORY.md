...
- **Windows 本地化(2026-08-07 现状)**: 当前 csproj **未**显式设置 `SatelliteResourceLanguages`(默认全文化)。8/5 曾尝试的 `SatelliteResourceLanguages`+`RedirectDotNetSatelliteToLocales`+`App.xaml.cs AssemblyLoadContext.Resolving`+`RedirectMauiImagesToResources`+`WindowsResourceFileImageSourceService` 五处优化**未保留在源码树**(8/5~8/7 间工作树被重置, 仅残留在 `bin/obj/dbg2` 编译产物)。WinUI 自包含 130+ `.mui` 文化目录仍原样保留(用户拍板: 安装后用户不看安装目录)。如需重做本地化收敛, 见 2026-08-05 讨论笔记(方案B: 保留 `locales\` 子目录 + 运行时 `Resolving` 钩子 + 删 PowerShell 兜底)。
...
## 插件系统架构(2026-08-07 现状)
- 设计=「宿主空壳, 插件自治」: 客户端不内置任何在线音源, 音源以独立 .dll 经「插件管理→安装」导入(`CatClawMusic.Plugins.Netease` 产出 `plugins/CatClawMusic.Plugins.Netease.ccp`)。原 `OnlineMusicPage` 已删, 由插件 `IViewContributorPlugin.CreateEntryPage` 提供入口页(返回 object 规避 Core 依赖 Maui)。
- 接口体系(`CatClawMusic.Core/Interfaces`): `IPlugin` 基 + `ILyricsProviderPlugin`/`IProtocolProviderPlugin`/`ICoverProviderPlugin`/`IAudioEnhancerPlugin`/`IMenuContributorPlugin`/`IOnlineMusicPlugin`/`IViewContributorPlugin`。
- `PluginManager`(Core/Services, ~1824 行): 生命周期 + 本地 `.ccp` 安装 + GitHub Release 下载 + 反射适配器(版本错配时按 FullName 反射包装) + `installed.json` 持久化 + 全局 `AssemblyResolve`。因插件运行时 `Assembly.Load` 独立程序集, csproj 已 `PublishTrimmed=false`(Android Release) 防裁剪破坏 typeref。
- `OnlineMusicAggregator`: 聚合所有 `IOnlineMusicPlugin`, 搜索结果按平台合并。
- 网易云 `NetEaseMusicPlugin`: 实现 `IOnlineMusicPlugin`+`IViewContributorPlugin`, 用 `NeteaseOpenApiClient`(官方 web API 匿名优先+可选 Cookie), 提供"网易云音乐"入口页(NeteaseOnlineMusicPage/VM)。登录走 `WebViewLoginPage`(Android CookieManager / Windows WebView2 CoreWebView2 / 回退 document.cookie), 由 `BrowserLoginInfo` 驱动。
