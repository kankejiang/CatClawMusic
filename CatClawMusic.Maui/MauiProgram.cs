...
/// <summary>
/// 跨平台 SafeArea 辅助：实现已上移至共享库 CatClaw.Shared.Maui.Helpers（与猫爪影视共用）。
/// 本类为静态转发占位（静态类不可继承），保持既有调用面——页面读 TopInset/BottomInset/LeftInset，
...
public static class SafeAreaHelper
{
    ...
    public static double LeftInset => CatClaw.Shared.Maui.Helpers.SafeAreaHelper.LeftInset;

    /// <summary>更新系统栏高度并触发事件（由平台代码调用）</summary>
    public static void UpdateInsets(double topDp, double bottomDp, double leftDp = 0)
        => CatClaw.Shared.Maui.Helpers.SafeAreaHelper.UpdateInsets(topDp, bottomDp, leftDp);
