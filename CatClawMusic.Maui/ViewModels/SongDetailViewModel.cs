...
public partial class SongDetailViewModel : ObservableObject
{
    ...
    public async Task LoadAsync(int songId)
    {
        ...
        try
        {
            ...
            if (song == null)
            {
                Log.Debug("SongDetailViewModel", $"[SongDetailVM] Song {songId} not found");
                return;
            }
            Song = song;
        ...
        {
            Log.Debug("SongDetailViewModel", $"[SongDetailVM] LoadAsync({songId}) failed: {ex}");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task LoadLyricsAsync(Song song)
    {
        ...
        {
            Log.Debug("SongDetailViewModel", $"[SongDetailVM] LoadLyrics failed: {ex.Message}");
        }
    }
