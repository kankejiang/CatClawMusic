...
public partial class LogEntryViewModel : ObservableObject
{
    ...
    /// <summary>级别简称（i/w/e，供 XAML 配色用）</summary>
    public string LevelCode => Level switch { LogLevel.Error => "e", LogLevel.Warn => "w", _ => "i" };
    /// <summary>级别标签文本（INFO/WARN/ERROR）</summary>
    public string LevelText => Level switch { LogLevel.Error => "ERROR", LogLevel.Warn => "WARN", _ => "INFO" };
...
public partial class LogViewModel : ObservableObject
{
    ...
    public void LoadLogs()
    {
        ...
        try
        {
            ...
            // 正则：HH:mm:ss.fff [L][tag] msg
            var regex = new Regex(@"^(\d{2}:\d{2}:\d{2}\.\d{3})\s*\[(I|W|E)\]\s*\[([^\]]+)\]\s?(.*)$");
    ...
    public async Task ShareDiagnosticPackageAsync()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[LogVM] Share failed: {ex.Message}");
        }
    }
}
