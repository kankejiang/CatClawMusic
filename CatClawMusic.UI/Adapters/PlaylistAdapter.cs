...
public class PlaylistAdapter : RecyclerView.Adapter
{
    ...
    public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
    {
        ...
        {
            // Create "new playlist" card view with theme-colored tint
            var density = ctx.Resources!.DisplayMetrics!.Density;
            ...
            var themePrimary = UiHelper.ResolveThemeColor(ctx, Resource.Attribute.catClawPrimaryColor, Color.ParseColor("#9B7ED8"));
            var tc = new Color(themePrimary);
            card.SetCardBackgroundColor(Color.Argb(13, tc.R, tc.G, tc.B)); // 5% opacity theme fill
            card.StrokeColor = Color.Argb(51, tc.R, tc.G, tc.B); // 20% opacity theme stroke
            ...
            card.Click += (s, e) => NewPlaylistClicked?.Invoke(this, EventArgs.Empty);
    ...
    public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
    {
        ...
        else if (holder is NewPlaylistViewHolder)
        {
            // Click handler is attached in OnCreateViewHolder to avoid duplicate subscriptions
        }
    }
