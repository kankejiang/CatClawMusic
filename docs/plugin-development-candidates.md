...
> 2026-08-12 进度：A4（LRCLIB 歌词插件）已完成 ✅ —— `CatClawMusic.Plugins.Lrclib`（纯 Core 依赖、宿主零改动，`.ccp` 已构建）。已迁出为独立仓库 [kankejiang/CatClawMusic.Plugins.Lrclib](https://github.com/kankejiang/CatClawMusic.Plugins.Lrclib)（MIT），不再随宿主仓维护。
> 2026-08-12 补充：`CatClawMusic.Plugins/` 已改造为**插件开发模板**（可编译的最小示例工程 + README 开发指南，含扩展点速查表/改名步骤/发布流程），仓库内不再内置任何具体插件。
...
| A4 | LRCLIB 歌词插件 | ILyricsProviderPlugin | 低 | ✅ 已完成（2026-08-12）：独立仓库 [kankejiang/CatClawMusic.Plugins.Lrclib](https://github.com/kankejiang/CatClawMusic.Plugins.Lrclib)（MIT）。按歌名/艺人/时长匹配 lrclib.net，同步歌词优先、纯文本兜底，内存缓存防限流；接口已消费、零实现者，装上即生效 |
