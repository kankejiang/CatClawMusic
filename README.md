...
跨平台音乐播放器（**Windows 桌面 / Android**），基于 **.NET MAUI**。

设计上**宿主是空壳、能力靠插件**：应用不内置任何 `.ccp` 音源插件，搜索 / 歌单 / 排行榜 /
播放直链 / 歌词等在线能力通过插件扩展；本地音乐库、播放内核、歌词引擎、
AI 助手等属于宿主自有功能。
...
| `CatClawMusic.Data` | **数据与服务实现层**（38 个文件）：`MusicDatabase`（SQLite，`catclaw.db`，**17 张表**，按歌曲 / 艺术家 / 歌单 / 收藏 / 歌词 / 播放历史与统计 / 网络缓存 / 聊天 / 迁移拆分为 9 个 partial 文件）、`MusicLibraryService`、`MusicScanner`、`NetworkMusicService`（扫描 / 流式 / 元数据三个 partial）、`SubsonicService`、`WebDavService`（连接测试 / 探测 / OpenList 探测）、`SmbService`、`CatClawServerClient`、`BackupService`（备份 / 恢复 / ZIP）、多源刮削（豆瓣 / 百度百科 / 网易云 / AI 艺术家 / 多源照片） |
| `CatClawMusic.Maui` | **宿主应用**：`Pages/`（65 个 `.cs` + 53 个 `.xaml`，含桌面 / 横屏专属形态）、`ViewModels/`（44 个）、`Services/`（52 个）、`Controls/`（35 个自绘控件 + 11 个 XAML）、`Converters/`（18 个）、`Platforms/`（41 个，Android / Windows 各一套实现） |
| `tests/CatClawMusic.Data.Tests` | 数据层单元测试（xUnit，4 个测试类 / 24 个 `[Fact]`）。`CatClawMusic.Core.Tests/` 是空目录残留 |
...
| `docs/` `design/` `release/` `release-assets/` | 设计文档、设计稿、出包产物与发布素材 |

> `docs/` 下除 `discover-page-design-spec.md` 外，`system_design.md`（2024 Xamarin→MAUI 迁移设计）、
> `known-issues.md`、`maui-android-follow-up-development-plan.md`、`resource-mapping.md` 均为**历史文档**，
> 不代表当前实现；`plugin-development-candidates.md`（2026-08）的"接口零实现"结论也已过时。
...
- **24 个内置工具**（`MauiProgram.cs` 注册）：音乐库 / 歌单类 10 个
  （`search_music`、`create_playlist`、`add_song_to_playlist`、`remove_song_from_playlist`、
  `list_playlists`、`get_playlist_songs`、`delete_playlist`、`get_favorite_songs`、
  `get_recent_songs`、`get_listening_stats`）、播放类 7 个（`play_song`、`control_playback`、
  `get_current_song`、`get_play_queue`、`toggle_favorite`、`add_to_play_queue`、
  `clear_play_queue`）、联网 / 下载类 6 个（`web_search`、`fetch_web_page`、`browser_open`、
  `download_file`、`netease_download`、`kuwo_download`）、音源 1 个（`update_music_source`）；
  只读工具并行执行、写类工具串行；
- **模型配置**：多份 `LlmConfig` 存在 MAUI `Preferences`（键前缀 `ai_`，`ai_all_configs`），
  内置 10 个 OpenAI 兼容服务商（deepseek / modelscope / llamacpp 本地 / zhipu / moonshot /
  qwen / spark / nvidia / opencodego / custom），支持备用模型、`ReasoningEffort`、
  上下文缓存等参数。**无视觉 / 嵌入（多模态）支持**；
- **记忆与人设**：`ai_memory.json`（最多 50 条，类别 user_profile / preference / personal / habit，
  每累计 20 条消息自动抽取）+ `wordlib.db`（Yuki 人格词库，未配置 LLM 时也能用本地词条回复）；
  上下文预算约 32 000 token，按"段"裁剪并修复 tool / tool_calls 配对；
- **Agent 浏览器**：`browser_open` 经 `AgentBrowserBridge` → `AgentBrowserCoordinator`，
  在聊天页顶部弹出 `AgentBrowserPreview` 小窗抓取页面（25 秒超时），不跳页。

### 音乐库

- 本地扫描（标题 / 艺术家 / 专辑 / 时长 / 封面，Android 另有 SAF 扫描路径）、
  按目录与标签浏览，全曲 / 歌手 / 专辑 / 歌单多视图；
- 歌单管理：新建 / 重命名 / 删除、批量移除歌曲、拖拽排序（`PlaylistDragDropHelper`）、
  收藏与播放历史；
...
> 宿主**没有内置标签编辑 / 批量整理 UI**：`TagReader` 的写方法（`WriteMetadata` /
> `WriteCoverToFile` / `WriteEmbeddedLyrics`）与 `IAudioFileService`、`ILoudnessAnalyzer`
> 都是为插件准备的能力出口，目前仓库内无宿主消费方。

### 播放

- **分平台音频内核**：Android 走 **AndroidX Media3 ExoPlayer**（渐进式磁盘缓存、
  唤醒锁 / Wi-Fi 锁、FFmpeg 软解兜底、前台服务 + MediaSession 通知栏控制）；
  Windows 走 **`Windows.Media.Playback.MediaPlayer`**（开启均衡器时切换为
  `WinEqualizerEngine` 的 AudioGraph + Biquad DSP 实时处理）；
- 播放队列 4 种模式（顺序 / 随机 / 单曲循环 / 列表循环，默认列表循环），
  随机模式 Fisher-Yates 洗牌并保持当前曲、支持跨重启恢复；上一曲走历史栈；
  **无 gapless**，仅有"提前缓存下一首网络音频"的预缓冲；
- 播放页含封面取色背景、多行歌词引擎（逐字 / 翻译 / 罗马音 / 卡拉OK 高亮）、
  全屏歌词页、横竖屏两套歌词布局；桌面歌词为平台分实现（Android 悬浮窗 /
  Windows 第二窗口 + DWM 透明化），另有 Windows 独占的**桌面歌词覆盖层**；
...
- 响度分析接口（`ILoudnessAnalyzer`，实现为 Android 的 FFmpeg `ebur128`）——**宿主目前无调用方**。
...
`release\windows\catclaw.music-<版本>-Setup.exe`（Inno Setup 脚本 `build-win-setup.iss` /
`installer.iss`，需安装 Inno Setup 7 的 `ISCC.exe`）。
...
产物分别为 `CatClawMusic.Maui\bin\Release\net11.0-android\com.catclaw.music-Signed.apk`
与同目录下的 `com.catclaw.music-x64-Signed.apk`；对外发布时复制为
`release\android\com.catclaw.music-<版本>-arm64.apk`。等价的手工命令：
...
> - 签名密钥为根目录 `catclaw.keystore`（别名 `catclaw`），已在 `CatClawMusic.Maui.csproj` 中配置，
>   Debug 与 Release 用同一签名以便交替安装。`SIGNING.md` 只保留密钥信息仍有效，
>   其中的命令行示例仍是旧的 `net10.0-android`，请以本文件与 `build-release.ps1` 为准。
...
`ai_memory.json`（AI 记忆）、`wordlib.db`（Yuki 词库）、`covers` / `artist_covers` /
`album_covers` / `defaultcovers`（封面缓存）、`theme_bg`、`bt`（BT 缓存）、`backups`、日志与崩溃转储。

插件安装记录位于 `Plugin\installed.json`，插件文件**按安装时的原始文件名**（如
`CatClawMusic.Plugins.Netease.ccp`）存放，不再追加时间戳。

## 已知缺口与开发注意事项

- **`Song.Duration` 现在契约统一为「秒」**（`TagReader` 写入与播放器回填均为秒），
  但历史写入路径曾混用毫秒，插件消费侧仍建议防御 `> 1000 ? ms : s`；
- `IAudioPlayerService` **没有** `CurrentSongChanged` 事件（只有 `PlaybackStateChanged` /
  `PositionChanged` / `DurationChanged` / `PlaybackCompleted`），做切歌联动需监听
  `PlaybackCompleted` + 轮询兜底；
- 播放**无 gapless**；交叉淡变是音量淡变模拟；
- Windows **SMTC 已禁用**（`CommandManager.IsEnabled=false`），系统媒体控制面板不生效；
- 响度只有**分析**能力（且仅 Android 经 FFmpeg 实现）、`ILoudnessAnalyzer` 与
  `IAudioFileService`（标签 / 封面 / 歌词写回）**在宿主内均无调用方**，
  ReplayGain 批量写入与播放时增益应用尚未实现；
...
- AI 助手虽属宿主功能，但其 `MusicSourceRegistry.DefaultSources()` **内置了酷我 / 网易云的接口配置**
  （供 `kuwo_download` / `netease_download` 工具直连），因此"宿主不内置任何在线音源"只对
  **`.ccp` 插件音源体系**成立，AI 下载链路是例外；
- `MauiProgram.cs` 交给备份服务的 AI 记忆文件名是 `ai_memory.md`，而运行时
  `ChatMemoryService` 实际读写 `ai_memory.json`，**该文件的备份 / 恢复当前不一致**；
...
| 仓库 | 说明 |
| --- | --- |
| [`CatClawMusic.Plugins.Netease`](https://github.com/kankejiang/CatClawMusic.Plugins.Netease) | 网易云音乐音源插件（`netEaseMusic` 0.4.0）：`IOnlineMusicPlugin` + `IViewContributorPlugin` + `ILyricsProviderPlugin` + `IQuickEntryPlugin` + `IMenuContributorPlugin` |
...
| [`CatClawMusic.Plugins.Lrclib`](https://github.com/kankejiang/CatClawMusic.Plugins.Lrclib) | LRCLIB / Lyrico 多源在线歌词插件（`lrclib` 1.2.0.9）：`ILyricsProviderPlugin` + `IViewContributorPlugin` + `IMenuContributorPlugin` |
| [`CatClawMusic.PluginMarket`](https://github.com/kankejiang/CatClawMusic.PluginMarket) | 插件市场**清单**仓库（非 .NET 工程）：全 GitHub 托管、零服务器，只存元数据，`.ccp` 本体在各作者仓库的 Release |
| [`catclaw-server`](https://github.com/kankejiang/catclaw-server) | 自托管音乐流媒体服务端（ASP.NET Core + EF Core / SQLite，net11.0）：提供 V0 `/api/*`、V1 `/api/v1/*` 与 **Subsonic 兼容 `/rest/*.view`** 三套 API，客户端按 Navidrome 协议接入。注意其 README 中的 ClawCircle / P2P / DHT 章节对应代码已删除 |
| `CatClaw.Shared` | 与猫爪影视共用的跨应用共享库（本地平级仓库，暂无远程地址） |
| [`CatClawVideo`](https://github.com/kankejiang/CatClawVideo) | **姊妹项目**：基于 .NET MAUI 的跨平台影视播放器，与本项目共用 `CatClaw.Shared` |

三个插件均以 `net11.0` 编译，Release 构建经 `PackageCcp` 产出 `.ccp`。
...
使用问题、功能建议、插件开发交流都欢迎进群；也可以在仓库 Issues 中反馈。
