...
| Tab 3 | 探索 | AI 对话式搜索/音乐搜索/模型管理 |
| Tab 4 | 音乐库 | 本地/网络 Tab 切换 |

**侧滑面板**：80% 宽度设置面板 + 20% 遮罩(60% 黑)，手势拖拽关闭(阈值 100px)，alpha 动画 250ms

**子页面**（Fragment 路由）：设置、通用设置、本地音乐设置、音乐文件夹、远程音乐、Navidrome 设置、WebDAV 设置、SMB 设置、桌面歌词设置、播放列表详情、模型管理、插件管理
...
- **AI 对话式搜索**：探索页面改为对话式布局，支持 AI 智能回复、音乐搜索卡片、工具调用
- **多供应商 AI 支持**：接入 8 个供应商（DeepSeek、魔搭社区、智谱 AI、Moonshot、通义千问、讯飞星火、llama.cpp、自定义 OpenAI 兼容）
- **模型管理**：独立的模型管理页面，支持查看、编辑、删除、启用/禁用 AI 模型
- **向导式添加模型**：4 步骤交互流程（选择服务商 → 输入 API Key → 选择模型 → 完成启用）
- **简单指令系统**：无需配置 AI 即可使用（播放、暂停、上一曲、下一曲、创建歌单、添加模型）
- **预设模型列表**：供应商不支持 `/v1/models` 端点时自动回退到预设模型列表
- **AI 搜索结果卡片**：AI 调用搜索工具后，结果以歌曲卡片列表形式展示，点击直接播放
- **回车键发送箭头图标**：重新设计的发送按钮

**🐛 修复**

- 修复魔搭社区 API 地址（`dashscope` → `api-inference.modelscope.cn`）
- 修复 DashScope 兼容模式获取模型列表 URL 错误
- 修复 AI 响应 JSON 解析 `null data` 崩溃（`ValueKind == JsonValueKind.Array` 检查）
- 修复搜索测试连接改用 chat completion 接口（原 `/v1/models` 端点部分供应商不支持）
- 修复桌面图标消失（MainActivity 缺少 MainLauncher 属性）
- 修复搜索结果覆盖输入框（`wrap_content` → `0dp` + `layout_weight`）
- 修复 ModelEditFragment 获取模型失败时回退到预设模型
- 修复发送按钮复用播放按钮图标（独立重绘）
- 修复切换供应商时自动变更 API 地址

**⚡ 优化**

- 探索页面 UI 重构：从实时搜索改版为对话式交互
- 配置名称自动填充：从 "服务商-模型" 自动生成
- 删除默认配置模型，支持完全自定义
